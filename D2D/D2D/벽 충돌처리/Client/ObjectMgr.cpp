...
void CObjectMgr::LateUpdate()
{
	...
	CCollisionMgr::CollisionTile(m_ObjectList[OBJECT_TERRAIN],m_ObjectList[OBJECT_PLAYER]);
}
	...
	while (true)
	{
		for (int i = OBJECT_BACK; i <= OBJECT_TERRAIN; ++i)
		{
			...
			for (int j = 0; j < size; ++j)
			{
				...
				if (OBJECT_BACK == i)
				{
					m_ObjectList[i].push_back(CBack::Create(tTile.vPos));
				}
				else if (OBJECT_DETAIL == i)
				{
					m_ObjectList[i].push_back(CDetail::Create(tTile.vPos, tTile.byDrawID));
				}
				else if (OBJECT_BREAKABLE == i)
				{
					m_ObjectList[i].push_back(CBreakable::Create(tTile.vPos, tTile.byDrawID));
				}
				else if (OBJECT_WALL == i)
				{
					m_ObjectList[i].push_back(CWall::Create(tTile.vPos));
				}
